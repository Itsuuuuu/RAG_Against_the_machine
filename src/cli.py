...
class CLI:
    """RAG against the machine: index a codebase, search it, answer."""
    ...
    def search_dataset(
        ...
        """Search a whole dataset, write a StudentSearchResults JSON."""
        try:
            bm25, chunks = self._load_index(index_path)
    ...
    def answer_dataset(
        ...
        """Generate answers for a StudentSearchResults JSON file."""
        try:
    ...
    def evaluate(
        ...
        print("  ".join(
            f"Recall@{k}: {value:.3f}" for k, value in recalls.items()
        ))
