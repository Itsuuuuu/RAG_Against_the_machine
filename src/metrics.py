...
def recall_at_k(results: StudentSearchResults, dataset: RagDataset, k: int) -> float:
...
def count_oversized_sources(results: StudentSearchResults) -> int:
	...
	return sum(
		...
		if source.last_character_index - source.first_character_index > MAX_CONTEXT_LENGTH
	)
